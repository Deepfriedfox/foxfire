...
% Fastball params
v = 70 * 0.44704;  % 70 mph to m/s ≈31.3
r_start = 20;  % Initial range m (start farther for multi-frame)
spin_rpm = 2000;  % RPM
r_ball = 0.0365;  % Baseball radius m
spin_rad_s = spin_rpm * 2*pi / 60;  % rad/s
v_spin = spin_rad_s * r_ball;  % Tangential vel m/s for micro-Doppler

% Open file for appending
filename = './testdata/dummy_fastball_session.bin';
...
for frame_idx = 0:num_frames-1
  % Update range for this frame (ball approaching)
  r_initial = r_start - v * frame_idx * frame_time;
  if r_initial < 0
    r_initial = 0;  % Stop at sensor
  end

  % Simulate samples for frame
  samples = zeros(1, num_chirps * num_samples);
  chirp_time = num_samples / sample_rate;  % Chirp duration s
  for chirp_idx = 0:num_chirps-1
    t_chirp_start = chirp_idx * prt;
    r = r_initial - v * t_chirp_start;  % Range at chirp start
    beat_freq = (2 * bw * r) / (c * chirp_time);  % Update beat for approach

    start_idx = chirp_idx * num_samples + 1;
    t_samples = linspace(0, chirp_time, num_samples);

    % Base chirp signal: Sine for beat freq
    chirp_sig = sin(2 * pi * beat_freq * t_samples) * 2000;

    % Doppler phase shift
    phase_doppler = 4 * pi * v * t_chirp_start / lambda;
    chirp_sig = chirp_sig .* cos(phase_doppler);

    % Micro-Doppler for spin
    mod_freq = v_spin / lambda;
    chirp_sig = chirp_sig + sin(2 * pi * mod_freq * t_samples) * 200;

    samples(start_idx : start_idx + num_samples - 1) = chirp_sig;
  end

  % Clip, shift to uint12
  ...
  % Serialize RawFrame (little-endian fixed)
  frame_id = frame_idx;  % u64
  timestamp_ns = uint64((time() + frame_idx * frame_time) * 1e9);  % Simulated increasing ns
  timestamp_low = bitand(timestamp_ns, 2^64 - 1);
  timestamp_high = bitshift(timestamp_ns, -64);
  raw_len = length(raw_data);  % u64
...
fclose(fid);
disp(['Wrote ', num2str(num_frames), '-frame dummy fastball session to ', filename]);
